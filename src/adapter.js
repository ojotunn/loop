...
export function realAdapter() {
  ...
  return {
    ...
    poolKeyFor: chain.poolKeyFor,
    quotePoolSell: ({ token, amountIn, deadline }) => chain.quotePoolSell({ token, from: agent, amountIn, deadline }),
    launch: signer('launch'),
    sell: signer('sell'),
    approveForPool: signer('approveForPool'),
    sellOnPool: signer('sellOnPool'),
    sweep: signer('sweep'),
    claim: signer('claim'),
    burn: signer('burn'),
  };
